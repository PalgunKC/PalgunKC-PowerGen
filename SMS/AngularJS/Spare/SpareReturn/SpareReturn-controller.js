SpareingreturnApp.controller('SpareingreturnCtrl', function (Excel, $timeout, $scope, $filter, SpareingreturnServices, commonService, DTOptionsBuilder, DTColumnBuilder) {
    ...
    $scope.obj = new SpareingreturnServices.SpareingreturnData(null);
    ...
    $scope.init = function () {
        ...
        commonService.postWebService('Spare/BindListSpareingReturn', { 'P_KEY': 'L', 'dummy_col1': 'Employee' }).then(function (response) {
    ...
    $scope.GetExcelData = function () {
        commonService.postWebService('Spare/GetExcelSpareReturnData', { 'P_KEY': 'EXCEL', 'dummycolumn1': 'Part' }).then(function (response) {
    ...
    $scope.getLineDetails = function (row) {
        debugger;
        var request = new SpareingreturnServices.getLineDetails(row);
    ...
    //$scope.filterNullUnicodeList == function (filterSpareUnicodeList) {
    //    return (filterSpareUnicodeList.SpareIsu_CurUnicode != null);
    //}
//    $scope.ChkUnicodeReturned = function (row) {
...
//}
    $scope.BindIsuId = function (row) {
        ...
        angular.forEach($scope.SpareIssueList, function (value, key) {
            ...
            //if (value.SpareIsu_Partno == row.SpareRts_Partno && value.SpareIsu_CurUnicode == row.SpareRts_Unicode) {
            if (value.PartId == row.PartId && value.SpareIsu_CurUnicode == row.SpareRts_Unicode) {
                i = '';
        ...
        //angular.forEach($scope.filterSpareUnicodeList, function (value, key) {
            //if (row.SpareRts_Unicode.SpareIsu_CurUnicode)
            //    {
            //if (value.SpareIsu_CurUnicode == row.SpareRts_Unicode.SpareIsu_CurUnicode) {
            //    row.SpareIsu_datetime = value.SpareIsu_datetime,
            ...
            //else
            //{
                //if (value.SpareIsu_CurUnicode == row.SpareRts_Unicode) {
                //    row.SpareIsu_datetime = value.SpareIsu_datetime,
                //    //row.SpareRqt_datetime = value.SpareReq_Date,
                //    row.SpareIsu_id = value.SpareIsu_id
                //    //$scope.filterSpareUnicodeList.push(item)
   ...
   $scope.AddTab_Visblity = function () {
       if ($scope.ReturnReason_Flag === false || $scope.Part_Flag === false || $scope.Location_Flag === false || $scope.Return_Flag === false || $scope.Vendor_Flag === false || $scope.Shift_Flag === false || $scope.Shift_Flag1 === false || $scope.Issue_Flag === false || $scope.Employee_Flag === false) {
           //return false;
           ...
           angular.forEach($scope.SpareingReturnList, function (value, key) {
               ...
               value.SpareRts_id = parseFloat(value.SpareRts_id);
           });
           angular.forEach($scope.SpareUploadList, function (value, key) {
   ...
   , 500)
    ...
    $scope.editClick = function (row, key, IsActive) {
        ...
        if (key == 'E') {
            ...
            $scope.obj = new SpareingreturnServices.SpareingreturnData(row);
            $scope.obj.SpareRts_Qty = parseFloat($scope.obj.SpareRts_Qty);
            $scope.obj.KEY = key;
 ...
 $scope.obj.SpareRts_Unicode = row.SpareRts_Unicode;
            ...
            $scope.obj.SpareRqt_datetime = datetime;
    ...
    $scope.submitForm = function (isValid, Spareingreturn) {
        ...
        if (isValid) {
            ...
            //Spareingreturn.SpareRts_Unicode = Spareingreturn.SpareRts_Unicode.SpareIsu_CurUnicode;
    ...
    $scope.SaveSpareingReturn = function (Spareingreturn) {
        ...
        var request = new SpareingreturnServices.SaveSpareingReturn(Spareingreturn);
        commonService.postWebService(request.url, request.param).then(function (response) {
            debugger;
            ...
            if (response.liSpareingReturn != null) {
                if (response.liSpareingReturn[0].MSG == "Updated Success") {
                    ...
                    $('#Title').html('Spare Return Saved Successfully');//Title
                    $scope.init();
                    $scope.obj = null;
                    $scope.SpareReturnForm.$setPristine();
                    $scope.CancelSpareingReturn();
