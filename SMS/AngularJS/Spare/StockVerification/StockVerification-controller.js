StockVerificationApp.controller('StockVerificationCtrl', function (Excel, $timeout,$scope, $filter, StockVerificationServices, commonService) {
    ...
    $scope.FunctionSearch = function (obj) {
        ...
        commonService.postWebService('Spare/BindListPartMasterStock', { 'P_KEY': 'Search', 'SearchField': obj.SearchField, 'SearchData': obj.SearchData }).then(function (response) {
            $scope.PartMasterList = response.liSparePartmaster;
    ...
    $scope.GetExcelData = function () {
        commonService.postWebService('Spare/GetExcelPartData', { 'P_KEY': 'VerificationList', 'dummycolumn1': 'Part' }).then(function (response) {
    ...
    $scope.BindFilterData = function () {
        ...
        commonService.postWebService('Spare/BindListPartMasterStock', { 'P_KEY': 'Part', 'P_PAGE_INDEX': $scope.currentPage, 'P_PAGE_SIZE': '10' }).then(function (response) {
            $scope.PartMasterList = response.liSparePartmaster;
            $scope.TotalCount = parseInt($scope.PartMasterList[0].TotalCount)
    ...
    //$scope.init = function () {
    //    debugger;
    //    commonService.postWebService('Spare/BindListPartMasterStock', {}).then(function (response) {
    //        $scope.PartMasterList = response.liSparePartmaster;
        ...
        commonService.postWebService(request.url, request.param).then(function (response) {
            ...
            if (response.liSparePartmaster != null) {
                if (response.liSparePartmaster[0].MSG == "Updated Success") {
                    ...
                    $('#Title').html('Spare StockVerification Updated Successfully');//Title
                ...
                $scope.StockVerificationForm.$setPristine();
                    ...
                    $('#Title').html('Spare StockVerification Saved Successfully');//Title
                    $("#Message").trigger("click");
                    $scope.init();
                    $scope.obj = null;
                    $scope.StockVerificationForm.$setPristine();
