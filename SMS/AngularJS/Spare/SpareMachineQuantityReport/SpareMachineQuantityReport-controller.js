SpareMachineQuantityReportApp.controller('SpareMachineQuantityReportCtrl', function (Excel, $timeout, $scope, $filter, SpareMachineQuantityReportServices, commonService, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder) {
    ...
    // var chart = '';
     $scope.FetchSpareMachineQuantity = function (data) {
         $scope.ReportType = data.Report_Type;
        // $scope.LineName = data.LineMachine_Line_Code;
        debugger;
        $scope.jsonObj = [];
        $scope.CostjsonObj = [];
        var request = new SpareMachineQuantityReportServices.FetchSpareMachineQuantity(data);
        commonService.postWebService(request.url, request.param).then(function (response) {
            ...
            angular.forEach($scope.ProductAnalyReportList, function (item) {
                $scope.jsonObj.push({ year: item.MachineName, value: item.Quantity })
            });
            $scope.CostAnalyReportList = response.ddlToolCostAnalyReport;
            angular.forEach($scope.CostAnalyReportList, function (item) {
                $scope.CostjsonObj.push({ date: item.MachineName, sales1: item.Quantity })
            });
            //$scope.s = [{ "year": "December", "value": "27", "value2": "24" }];
            $scope.obj = new SpareMachineQuantityReportServices.SpareMachineQuantityReportData(data);
    ...
    $scope.GetQuantityReport = function (QuantityReport) {
        ...
        var request = new SpareMachineQuantityReportServices.FetchSpareMachineQuantityExcel(QuantityReport);
        commonService.postWebService(request.url, request.param).then(function (response) {
            debugger;
    ...
    $scope.GetAnalysisReport = function (QuantityReport) {
        var chart = AmCharts.makeChart("line-area2", {
            ...
            "graphs": [{
                "id": "g1",
                "title": "Quantity ",
                "balloonText": "[[category]]<br><b><span style='font-size:14px;'>[[value]] Quantity</span></b>",
                "bullet": "round",
                "lineColor": "#1de9b6",
                "lineThickness": 3,
