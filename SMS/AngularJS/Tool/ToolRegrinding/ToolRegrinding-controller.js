ToolingregrindApp.controller('ToolingregrindCtrl', function (Excel, $timeout, $scope, $filter, ToolingregrindServices, commonService, DTOptionsBuilder, DTColumnBuilder) {
    ...
    $scope.init = function () {
        debugger;
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Part' }).then(function (response) {
            $scope.PartMasterList = response.ddlPartmaster;
            $scope.Part_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Return' }).then(function (response) {

            $scope.ToolingReturnList = response.ddToolingReturn;
            $scope.Return_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Vendor' }).then(function (response) {

            $scope.ToolVendorList = response.ddlVendormaster;
            $scope.Vendor_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Shift' }).then(function (response) {

            $scope.ShiftMasterList = response.ddlShiftmaster;
            $scope.ToolRts_plant = response.ddlOrgId;
            $scope.Shift_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Line' }).then(function (response) {
            $scope.LineList = response.ddlLine;
            $scope.Line_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Issue' }).then(function (response) {
            $scope.ToolIssueList = response.ddlToolingIssue;
            $scope.Issue_Flag = true;
        });
        commonService.postWebService('Tool/BindListRegrinding', { 'P_KEY': 'L', 'dummy_col1': 'Upload' }).then(function (response) {
    ...
    var intervalId = window.setInterval(
      $scope.ShowTab = function () {
          if ($scope.Part_Flag === false || $scope.Return_Flag === false|| $scope.Vendor_Flag === false|| $scope.Shift_Flag === false || $scope.Line_Flag === false || $scope.Issue_Flag === false || $scope.Upload_Flag === false) {
              $scope.disableTabCtrl = true;
          }
          else {
...
, 500)
     ...
     $scope.GetExcelData = function () {
        commonService.postWebService('Tool/GetExcelToolPartRegrindingData', { 'P_KEY': 'EXCEL', 'dummycolumn1': 'Part' }).then(function (response) {
